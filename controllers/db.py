...
class Mysql:
    ...
    Args:
        ...
        authentication (obj); authentication object
    """
    def __init__(self, session, authentication):
        load_dotenv()
        self.engine = self._create_engine()
        Base.metadata.create_all(bind=self.engine)
        self.db_session = self._create_session()
    ...
    def number_of(self, model):
        self.db_session.rollback()
        query = self.db_session.query(self._get_model_class(model))
    ...
    def _apply_join(self, query):
        if not self._want_all() and self.session.connected_user.department_id == 1:
    ...
    def _contract_filter(self, query):
        if not (self.session.client.id is None or self._want_all()):
    ...
    def _event_filter(self, query):
        if not self._want_all() and self.session.connected_user.department_id == 2:
